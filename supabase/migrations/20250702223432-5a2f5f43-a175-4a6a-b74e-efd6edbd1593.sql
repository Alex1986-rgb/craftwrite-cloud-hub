...
-- 1. Fix the diagnostics function to handle JSONB properly
...
BEGIN
  ...
  SELECT 
    ...
    'orders_system',
    CASE 
      WHEN COUNT(*) FILTER (WHERE status = 'pending') < 5 THEN 'pass'
      WHEN COUNT(*) FILTER (WHERE status = 'pending') < 15 THEN 'warning'
      ELSE 'fail'
    END,
    jsonb_build_object(
      'total_orders', COUNT(*),
      ...
      'recent_orders', COUNT(*) FILTER (WHERE created_at >= now() - interval '24 hours'),
      'timestamp', now()::text
    )
  FROM public.orders
...
SELECT 'Testing diagnostics function:' as test_status;
SELECT check_type, check_name, status FROM public.run_system_diagnostics();