...
export default function OrderPageHero({ onQuickOrder }: OrderPageHeroProps) {
  return (
    <section className="text-center mb-12 md:mb-20 animate-fade-in" role="banner" aria-labelledby="hero-heading">
      <header className="mb-6 md:mb-8">
        <div className="inline-flex items-center gap-2 md:gap-3 bg-gradient-to-r from-blue-100/80 to-purple-100/80 text-blue-700 px-4 py-2 md:px-8 md:py-4 rounded-full text-xs md:text-sm font-bold border border-blue-200/50 shadow-lg backdrop-blur-sm">
          <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" aria-hidden="true"></div>
          <Star className="w-4 h-4 md:w-5 md:h-5" aria-hidden="true" />
          <span className="hidden sm:inline">Элитная команда</span> SEO-экспертов
          <div className="w-2 h-2 bg-purple-500 rounded-full animate-pulse" style={{ animationDelay: '0.5s' }} aria-hidden="true"></div>
        </div>
      </header>
      
      <h1 
        id="hero-heading"
        className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-black mb-6 md:mb-8 bg-gradient-to-r from-slate-900 via-blue-800 to-purple-800 bg-clip-text text-transparent leading-tight tracking-tight px-2"
      >
        Каталог услуг <br />
      ...
      <section className="flex flex-wrap justify-center items-center gap-3 md:gap-6 mb-6 md:mb-10 px-4" aria-label="Наши преимущества">
        <div className="flex items-center gap-2 md:gap-3 bg-gradient-to-r from-emerald-50 to-green-50 text-emerald-700 px-3 py-2 md:px-6 md:py-3 rounded-full text-xs md:text-sm font-semibold border border-emerald-200/50 shadow-lg backdrop-blur-sm">
          <Shield className="w-3 h-3 md:w-4 md:h-4" aria-hidden="true" />
      ...
      <Button 
        ...
        aria-label="Перейти к быстрому оформлению заказа"
      >
        <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent rounded-full blur-sm" aria-hidden="true"></div>
        <Sparkles className="w-4 h-4 md:w-5 md:h-5 mr-2 relative z-10" aria-hidden="true" />
